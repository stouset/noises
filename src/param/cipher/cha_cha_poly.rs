...
mod tests {
    ...
    fn test_vector_getkey_1() {
        ...
        let     nonce  =  b"\x00\x00\x00\x00\x00\x00\x00\x00";
        let     vector =  b"\x76\xb8\xe0\xad\xa0\xf1\x3d\x90\x40\x5d\x6a\xe5\x53\x86\xbd\x28\xbd\xd2\x19\xb8\xa0\x8d\xed\x1a\xa8\x36\xef\xcc\x8b\x77\x0d\xc7";

        test_vector_getkey(&mut key, &nonce, vector);
    }

    #[test]
    fn test_vector_getkey_2() {
        let mut key    = *b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01";
        let     nonce  =  b"\x00\x00\x00\x00\x00\x00\x00\x00";
        let     vector =  b"\x45\x40\xf0\x5a\x9f\x1f\xb2\x96\xd7\x73\x6e\x7b\x20\x8e\x3c\x96\xeb\x4f\xe1\x83\x46\x88\xd2\x60\x4f\x45\x09\x52\xed\x43\x2d\x41";

        test_vector_getkey(&mut key, &nonce, vector);
    }

    #[test]
    fn test_vector_getkey_3() {
        let mut key    = *b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
        let     nonce  =  b"\x00\x00\x00\x00\x00\x00\x00\x01";
        let     vector =  b"\xde\x9c\xba\x7b\xf3\xd6\x9e\xf5\xe7\x86\xdc\x63\x97\x3f\x65\x3a\x0b\x49\xe0\x15\xad\xbf\xf7\x13\x4f\xcb\x7d\xf1\x37\x82\x10\x31";

        test_vector_getkey(&mut key, &nonce, vector);
    }

    #[test]
    fn test_vector_getkey_4() {
        let mut key    = *b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
        let     nonce  =  b"\x01\x00\x00\x00\x00\x00\x00\x00";
        let     vector =  b"\xef\x3f\xdf\xd6\xc6\x15\x78\xfb\xf5\xcf\x35\xbd\x3d\xd3\x3b\x80\x09\x63\x16\x34\xd2\x1e\x42\xac\x33\x96\x0b\xd1\x38\xe5\x0d\x32";

        test_vector_getkey(&mut key, &nonce, vector);
    }

    #[test]
    fn test_vector_getkey_5() {
        let mut key    = *b"\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f";
        let     nonce  =  b"\x00\x01\x02\x03\x04\x05\x06\x07";
        let     vector =  b"\xf7\x98\xa1\x89\xf1\x95\xe6\x69\x82\x10\x5f\xfb\x64\x0b\xb7\x75\x7f\x57\x9d\xa3\x16\x02\xfc\x93\xec\x01\xac\x56\xf8\x5a\xc3\xc1";
    ...
    fn test_vector_getkey(key: &mut [u8; 32], nonce: &[u8; 8], vector: &[u8; 32]) {
        let key    = Key::from(key);
        let result = ChaChaPoly::getkey(&key, nonce);
